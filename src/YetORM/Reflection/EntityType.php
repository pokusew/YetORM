...
class EntityType extends NClassType
{
	...
	private function loadMethodProperties()
	{
		...
		{
			if ($method->declaringClass->name !== 'YetORM\\Entity' && strlen($method->name) > 3 && substr($method->name, 0, 3) === 'get' && !$method->hasAnnotation('internal'))
			{
				...
				$this->properties[$name] = new MethodProperty(
						$this, $name, !$this->hasMethod($setter), $type, $getter, $setter
				);
			}
		}
	}
					...
					foreach ($values as $tmp)
					{
						...
						if (count($split) >= 2)
						{
							...
							self::$annProps[$class][$name] = new AnnotationProperty(
									$ref, $name, $readonly, $type, $column, $nullable
							);
						}
					}
				}
